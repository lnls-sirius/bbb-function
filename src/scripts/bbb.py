...
class BBB:
    ...
    def change_ip_address(self, dhcp_manual, new_ip_address="", new_mask="", new_gateway=""):
        ...
        if new_ip_address != "":
            self.logger.info(
                "Changing current IP address from {} to {}".format(self.get_network_specs()[1], new_ip_address)
            )
    ...
    def update_nameservers(self, nameserver_1="", nameserver_2=""):
        ...
        subprocess.check_output(
            ["connmanctl config {} --nameservers {} {}".format(service, nameserver_1, nameserver_2)], shell=True
        )
    ...
    def get_connman_service_name(self):
        ...
        """
        services = subprocess.check_output(["connmanctl services"], stderr=subprocess.STDOUT, shell=True).decode(
            "utf-8"
        )
        ...
        for service in services.split():
            ...
            if service.startswith("ethernet_"):
                service_properties = subprocess.check_output(
                    ["connmanctl services " + service], stderr=subprocess.STDOUT, shell=True
                ).decode("utf-8")
...
class Sector:
    ...
    SUBNETS = (
        ...
        + [ipaddress.ip_network("10.128.{}.0/24".format(i)) for i in range(101, 124)]
